...
function requireFile(base, file) {
  existsSync(resolve(base, file)) ? pass(`required file: ${file}`) : fail(`missing required file: ${file}`);
}
function textFiles(directory) {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    else if (['.html', '.js', '.css', '.svg', '.xml', '.txt', '.md', '.json'].includes(extname(entry.name))) files.push(path);
  }
  return files;
}
function scan(directory, patterns, label) {
  ...
  for (const file of textFiles(directory)) {
    ...
    const content = label === 'claim scan'
      ? raw.replace(/Does LANVEXA replace a cable tester\?<\/summary>\s*<div>\s*<p>No\./gi, '')
      : raw;
...
function build(outDir, env = {}) {
  ...
  if (result.status !== 0) fail(`metadata test build failed:\n${result.stderr || result.stdout}`);
  return result.status === 0;
}
function checkNoDomainBuild(directory) {
  const html = readFileSync(resolve(directory, 'index.html'), 'utf8');
  if (/rel="canonical"|property="og:url"/.test(html)) fail('empty-domain build emitted canonical or og:url');
  else pass('empty-domain build omits canonical and og:url');
  if (existsSync(resolve(directory, 'sitemap.xml'))) fail('empty-domain build emitted sitemap.xml');
  else pass('empty-domain build omits sitemap.xml');
  const robots = readFileSync(resolve(directory, 'robots.txt'), 'utf8');
  if (/Sitemap:/i.test(robots)) fail('empty-domain robots.txt advertises a sitemap');
...
function checkDomainBuild(directory, origin) {
  ...
  for (const page of indexablePages) {
    ...
    if (!html.includes(`rel="canonical" href="${url}"`)) fail(`incorrect canonical: ${page}`);
    if (!html.includes(`property="og:url" content="${url}"`)) fail(`incorrect og:url: ${page}`);
    if (!html.includes(`content="${origin}/lanvexa-social.png"`)) fail(`social image is not absolute: ${page}`);
  }
  ...
  for (const url of listed) {
    ...
    if (!existsSync(resolve(directory, output))) fail(`sitemap route has no emitted file: ${route}`);
  }
  const robots = readFileSync(resolve(directory, 'robots.txt'), 'utf8');
  robots.includes(`Sitemap: ${origin}/sitemap.xml`) ? pass('robots.txt references the generated sitemap') : fail('robots.txt sitemap reference is incorrect');
  const builtJs = textFiles(resolve(directory, 'assets')).filter((file) => extname(file) === '.js').map((file) => readFileSync(file, 'utf8')).join('\n');
  builtJs.includes('fetch(') ? pass('form-enabled build retains the Netlify submission path') : fail('form-enabled build removed the Netlify submission path');
...
if (!existsSync(dist)) fail('dist does not exist; run npm run build first');
  ...
  pngWidth === 1200 && pngHeight === 630 ? pass('social PNG is 1200 × 630') : fail(`social PNG is ${pngWidth} × ${pngHeight}`);
  ...
  const emailMatches = textFiles(dist).flatMap((file) => {
    ...
    return [...content.matchAll(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi)].map((match) => `${relative(dist, file)}: ${match[0]}`);
  });
  const approvedEmails = new Set(Object.values(liveConfig.contacts).filter(Boolean).map((email) => email.toLowerCase()));
...
const headerChecks = [
  ['CSP keeps scripts and styles self-hosted', /Content-Security-Policy = "[^"]*style-src 'self'; script-src 'self';[^"]*form-action 'self'/],
  ...
  ['HTML/default revalidation', /Cache-Control = "public, max-age=0, must-revalidate"/],
  ['immutable hashed assets', /for = "\/assets\/\*"[\s\S]*?max-age=31536000, immutable/],
  ['short-lived media cache', /for = "\/media\/\*"[\s\S]*?max-age=86400/],
];
for (const [name, pattern] of headerChecks) pattern.test(netlify) ? pass(name) : fail(`Netlify configuration: ${name}`);

for (const invalidOrigin of ['http://lanvexa.example', 'https://localhost', 'https://example.net']) {
  try {
...
if (build(noDomain, { LANVEXA_SITE_URL: '', LANVEXA_METADATA_TEST: '1' })) checkNoDomainBuild(noDomain);
