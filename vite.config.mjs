...
function contactMarkup(kind, label) {
  ...
  return email
    ? `<p class="public-contact"><strong>${label}</strong> <a href="mailto:${email}">${email}</a></p>`
    : "";
}
...
function identityMarkup() {
  return [
    config.publisherName
      ? `<p class="publisher-identity">Published by ${config.publisherName}</p>`
      : "",
    config.contacts.support
      ? `<p class="publisher-identity"><a href="mailto:${config.contacts.support}">Support contact</a></p>`
      : "",
  ].join("");
}
...
function authenticMedia() {
  ...
  const screenshot = existsSync(
    resolve(media, "lanvexa-product-screenshot.png"),
  );
  if (poster && (webm || mp4)) {
    const sources = [
...
const launchTransforms = {
  ...
  transformIndexHtml: {
    ...
    handler(html, context) {
      const media =
        context.path === "/index.html" || context.path === "/"
          ? authenticMedia()
          : "";
      let transformed = html
        .replace(
          "<!-- LANVEXA_SECURITY_CONTACT -->",
          contactMarkup("security", "Security contact:"),
        )
        .replace(
          "<!-- LANVEXA_PRIVACY_CONTACT -->",
          contactMarkup("privacy", "Privacy contact:"),
        )
        .replace(
          "<!-- LANVEXA_SUPPORT_CONTACT -->",
          contactMarkup("support", "Support contact:"),
        )
        ...
        .replace(
          ...
          media
            ? "data-interactive-fallback hidden"
            : "data-interactive-fallback",
        );
      ...
      transformed = transformed
        .replace(
          /content="\/lanvexa-social\.png"/g,
          `content="${config.siteUrl}/lanvexa-social.png"`,
        )
        .replace(
          "</head>",
  ...
  build: {
    rollupOptions: {
      input: {
        ...
        connectivity: resolve(root, "docs/connectivity-checks.html"),
        discovery: resolve(root, "docs/lldp-cdp-discovery.html"),
        protocolComparison: resolve(root, "docs/lldp-vs-cdp.html"),
        switchPort: resolve(root, "docs/identify-switch-port-windows.html"),
